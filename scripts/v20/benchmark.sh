#!/usr/bin/env bash
set -euo pipefail
ROOT=${MAINCHK_ROOT:-/home/service/llama}; PROFILES=${*:-main openvino llama-json}; STAMP=$(date +%Y%m%d-%H%M%S); OUT="$ROOT/results/v20-$STAMP"; READY=/run/maincheck-v20-ready; STATUS=/run/maincheck-v20-install.status; mkdir -p "$OUT"
...
for p in $PROFILES; do
  sudo -E "$ROOT/scripts/v20/install.sh" "$p"; wait_ready "$p"
