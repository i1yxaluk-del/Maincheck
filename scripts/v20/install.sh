...
set -euo pipefail
PROFILE=${1:-main}
ROOT=${MAINCHK_ROOT:-/home/service/llama}
...
UNIT=ai-suggester-v20-install-${PROFILE//[^a-zA-Z0-9]/-}
if [[ ${EUID:-$(id -u)} -ne 0 ]]; then echo "run with sudo" >&2; exit 1; fi
systemd-run --unit="$UNIT" --collect --property=Type=exec --property=StandardOutput=append:"$LOG" --property=StandardError=append:"$LOG" /bin/bash "$ROOT/scripts/v20/install-worker.sh" "$PROFILE"
