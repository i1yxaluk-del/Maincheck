...
apt_safe_install(){
  ...
  { grep -hE '^[[:space:]]*deb[[:space:]]' /etc/apt/sources.list /etc/apt/sources.list.d/*.list 2>/dev/null || true; } | grep -Ev 'cdrom:|repository-devel' >"$src"
  [[ -s "$src" ]] || { rm -f "$src"; log 'no usable apt repositories after filtering cdrom and repository-devel'; return 1; }
  local opts=(-o "Dir::Etc::sourcelist=$src" -o "Dir::Etc::sourceparts=-" -o "APT::Get::List-Cleanup=0")
  apt-get "${opts[@]}" update
  DEBIAN_FRONTEND=noninteractive apt-get "${opts[@]}" install -y "$@"
  rm -f "$src"
}
pkg_build(){ if command -v dnf >/dev/null; then dnf install -y git cmake gcc-c++ make curl; elif command -v apt-get >/dev/null; then apt_safe_install git cmake build-essential curl ca-certificates; else log 'no supported package manager'; return 1; fi; }
pkg_venv(){ if command -v dnf >/dev/null; then dnf install -y python3; elif command -v apt-get >/dev/null; then apt_safe_install python3-venv; else return 1; fi; }
...
case "$PROFILE" in
  ...
  openvino)
    ...
    ;;
  llama-json)
    if [[ ! -x "$LJ_VENV/bin/python" ]]; then python3 -m venv "$LJ_VENV" || { pkg_venv; python3 -m venv "$LJ_VENV"; }; chown -R service:service "$LJ_VENV"; fi
    "$LJ_VENV/bin/pip" install --disable-pip-version-check -r "$LOCAL/requirements-v20-llama-json.txt"
    ;;
esac
if [[ "$PROFILE" == llama-json ]]; then
  if ! command -v git >/dev/null || ! command -v cmake >/dev/null || ! command -v g++ >/dev/null || ! command -v make >/dev/null || ! command -v curl >/dev/null; then pkg_build; fi
  if [[ ! -x "$LLAMA_DIR/build/bin/llama-server" ]]; then
    [[ -d "$LLAMA_DIR/.git" ]] || runuser -u service -- git clone --depth 1 https://github.com/ggml-org/llama.cpp.git "$LLAMA_DIR"
    runuser -u service -- cmake -S "$LLAMA_DIR" -B "$LLAMA_DIR/build" -DGGML_NATIVE=ON -DGGML_BLAS=OFF -DLLAMA_CURL=OFF
...
THREADS_DEFAULT=$(( $(nproc) / 2 )); (( THREADS_DEFAULT > 0 )) || THREADS_DEFAULT=1
...
V20_LLAMA_THREADS=${V20_LLAMA_THREADS:-$THREADS_DEFAULT}
