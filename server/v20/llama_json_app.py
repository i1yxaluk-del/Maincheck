...
class LlamaJsonEngine:
    ...
    def __init__(self):
        self.url=os.getenv('V20_LLAMA_URL','http://127.0.0.1:8091').rstrip('/'); self.model=os.getenv('V20_LLAMA_MODEL_NAME','local-gec'); self.timeout=float(os.getenv('V20_LLAMA_TIMEOUT','25'))
    ...
    async def correct(self,text,context):
        system='Ты корректор русского официального текста. Верни только JSON-массив локальных правок: [{"start":0,"end":1,"before":"...","after":"...","reason":"...","confidence":0.9}]. Смещения символов относятся к исходному тексту. Не переписывай корректные фрагменты, названия, числа и переносы.'
        user=f'Контекст:\n{context[-1200:]}\n\nТекст:\n{text}'
        ...
        for x in data if isinstance(data,list) else data.get('edits',[]):
