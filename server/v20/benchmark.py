...
def run(url,cases):
 ...
 for c in cases:
  ...
  layout=corrected.count('\n')==c['text'].count('\n');clean_ok=(corrected==c['text']) if c.get('clean') else True;reason_ok='bounded token diff' not in payload.lower()
  rows.append({'id':c['id'],'category':c.get('category','unspecified'),'source':c.get('source','unspecified'),'clean':bool(c.get('clean')),'pass':bool(not error and not missing and not forbidden and not response_missing and not response_forbidden and layout and clean_ok and reason_ok),'layout_ok':layout,'reason_ok':reason_ok,'changed':corrected!=c['text'] if corrected else False,'latency_ms':round(ms,1),'missing_required':missing,'present_forbidden':forbidden,'response_missing':response_missing,'response_forbidden':response_forbidden,'changes':_changes(payload),'corrected':corrected,'error':error})
