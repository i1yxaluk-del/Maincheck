...
def run(url,cases):
    ...
    for c in cases:
        ...
        rows.append({'id':c['id'],'clean':bool(c.get('clean')),'pass':bool(not error and required and forbidden and layout and clean_ok),'layout_ok':layout,'changed':corrected!=c['text'] if corrected else False,'latency_ms':round(ms,1),'error':error})
