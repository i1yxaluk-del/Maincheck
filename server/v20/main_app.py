...
import os,re
# The production entrypoint must not inherit the legacy A preset from .env.
os.environ['LLM_PRESET']='Z'
...
_legal=LegalStyleStage();_rules=router.rules.candidates
router.rules.candidates=lambda text:_rules(text)+_legal.candidates(text)
...
def _allowed(text,c):
 ...
 generative=is_generative(c.category);punct=is_punctuation_only(c);pos=_start(text,c)
...
async def _production_candidates(text,context=''):
 candidates=await _base_candidates(text,context);safe=[c for c in candidates if _allowed(text,c)]
...
def _metrics():
 result=_base_metrics();result['legal_style']=_legal.metrics();result['v20_production_safety']=dict(_safety);result['v20_reasoning_timeout']=float(os.environ['REASONING_TOTAL_TIMEOUT']);return result
router.metrics=_metrics
app.title='Maincheck production: punctuation, spelling, grammar and legal style'
app.version='20-main-production.2'
