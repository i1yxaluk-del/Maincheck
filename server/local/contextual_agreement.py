...
class ContextualAgreementRules:
 ...
 @staticmethod
 def _verb_features(parses):
  return {(p.tag.number,p.tag.gender) for p in parses if p.tag.POS=='VERB' and p.normal_form in COPULAS}
 def candidates(self,text):
  ...
  for vi,verb in enumerate(words):
   vparses=[p for p in self.morph.known_parses(verb.group()) if p.tag.POS=='VERB' and p.normal_form in COPULAS]
   vf=self._verb_features(vparses)
   if not vf:continue
   subject=None
   for si in range(vi-1,max(-1,vi-15),-1):
    ...
    nouns=[p for p in self.morph.noun_parses(words[si].group()) if p.tag.case=='nomn']
    nouns=[p for p in nouns if any(p.tag.number==n and (n=='plur' or g is None or p.tag.gender==g) for n,g in vf)]
   ...
   for ai in range(vi+1,min(len(words),vi+5)):
    if re.search(r"[.!?;,]",text[verb.end():words[ai].start()]):break
    parses=[p for p in self.morph.attributive_parses(words[ai].group()) if p.tag.POS in {'ADJF','PRTF'} and p.tag.case=='ablt']
    ...
    forms={f.word for f in (p.inflect(grammemes) for p in parses[:8]) if f and f.word and f.word.casefold()!=words[ai].group().casefold()}
    if len(forms)==1:
     source=words[ai].group();fixed=preserve_capitalization(source,preserve_yo(source,forms.pop()))
     out.append(EditCandidate(source,fixed,0.992,'rule-copular-agreement',f'именная часть сказуемого согласуется с подлежащим «{words[si].group()}»',start=words[ai].start()))
    break
  return out
