...
def _global_rewrite(source: str, corrected: str) -> bool:
    """Reject drafts that alter the sentence globally instead of locally.

    Local GEC may change several words, but a draft that substantially changes
    character volume or word count is more likely a rewrite/paraphrase.
    """
    source_words = len(WORD_RE.findall(source))
    corrected_words = len(WORD_RE.findall(corrected))
    if source_words and abs(corrected_words - source_words) > 1:
        return True
    baseline = max(1, min(len(source), len(corrected)))
    ratio = abs(len(corrected) - len(source)) / baseline
    return ratio > 0.20
