...
def test_blagodarya():
 stage=GovernmentFrameStage();assert 'благодаря принятым мерам' in apply(stage,'Нарушение устранено благодаря принятых мер.');assert apply(stage,'Согласно приказа директора работа продолжена.')=='Согласно приказу директора работа продолжена.'
...
def test_deepfix():
 frames=GovernmentFrameStage();assert apply(frames,'Работу завершили благодаря своевременных организационных мер.')=='Работу завершили благодаря своевременным организационным мерам.';assert apply(frames,'Документы оформлены согласно утвержденного руководителем порядка делопроизводства.')=='Документы оформлены согласно утвержденному руководителем порядку делопроизводства.';assert 'и ведении журнала' in apply(frames,'При несениях караульной службы и ведений журнала выявлены недостатки.');assert apply(frames,'Документы поступившие вчера была зарегистрированы в журнале.')=='Документы поступившие вчера были зарегистрированы в журнале.';assert apply(frames,'Проверкой установлено, что отсутствует порядок ведений учёта.')=='Проверкой установлено, что отсутствует порядок ведения учёта.'
 legal=LegalStyleStage();text='Проверкой установлено, что в отношениях Центра в отношении Центра отсутствует порядок.';assert apply(legal,text)=='Проверкой установлено, что в отношении Центра отсутствует порядок.'
 contextual=ContextualAgreementRules();clean='Вследствие аварии сервер был временно отключён от локальной вычислительной сети.';assert apply(contextual,clean)==clean;assert apply(contextual,'Входная дверь оставалась открытыми настежь.')=='Входная дверь оставалась открытой настежь.'
def test_clean():
