"""Evaluate the fast punctuation ensemble on official Russian prose."""
...
async def run():
    model = RuPunctStage()
    structural = StructuralPunctuationRules(get_morphology())
    outcomes = []
    for line in CASES.read_text(encoding="utf-8").splitlines():
        case = json.loads(line)
        candidates = structural.candidates(case["text"])
        candidates += await model.candidates(case["text"])
...
def main() -> int:
    ...
    failures = [{"id": c["id"], "result": r, "expected": c["expected"]}
                for c, r in outcomes if r != c["expected"]]
