...
NORMALIZED_RELATION_REPEAT=re.compile(r"\bв\s+отношениях\s+(?P<object>[А-Яа-яЁё-]+)(?P<repeat>\s+в\s+отношении\s+(?P=object))",re.IGNORECASE)
...
class LegalStyleStage:
 ...
 def _duplicates(self,text):
  ...
  for m in NORMALIZED_RELATION_REPEAT.finditer(text):
   start,end=m.start('repeat'),m.end('repeat');out.append(EditCandidate(text[start:end],'',1.0,'rule-legal-repetition','Удалён повтор нормализуемой конструкции «в отношении».',start=start,sources=('rule-legal-repetition',)));occupied.append((start,end));self.found['repetition']+=1
 ...
 def _government(self,text):
  ...
  for m in RELATION_ERROR.finditer(text):
   ...
   source='отношениях';start=m.start()+m.group(0).lower().find(source);out.append(EditCandidate(text[start:start+len(source)],_case_like(text[start:start+len(source)],'отношении'),1.0,'rule-legal-government','В значении «касательно объекта» употребляется «в отношении».',start=start,sources=('rule-legal-government',)));self.found['government']+=1
  return out
 ...
 def _legal_punctuation(self,text):
  out=[]
  for m in AMOUNT_COMMA.finditer(text):out.append(EditCandidate(',', '',1.0,'rule-legal-punctuation','Удалена лишняя запятая: оборот «на сумму …» связан со сказуемым и не обособляется.',start=m.start('comma'),sources=('rule-legal-punctuation',)));self.found['legal_punctuation']+=1
  return out
