...
### Cloud: «исчерпана дневная квота free-tier OpenRouter» (HTTP 429)

Free-tier OpenRouter имеет общую дневную квоту на ВСЕ модели с
суффиксом `:free` (~50 запросов/день без баланса, **1000 запросов/день**
при пополнении баланса на ≥$10). Когда квота исчерпана — все `:free`
модели одновременно возвращают HTTP 429, и fallback по `:free` не
помогает.

Сервер в этом случае отвечает понятным `OpenRouterError` с инструкцией.
Варианты решения:
1. **Пополнить баланс OpenRouter** на $10 → 1000 запросов/день по всем
   `:free` моделям. Самый простой путь для ведомства, где сервер
   общий на много пользователей.
2. **Подождать 24 часа** — квота сбрасывается. Подходит для разовых
   проб.
3. **Прописать платную модель** в `server/cloud/.env`:
   ```
   OPENROUTER_MODELS=meta-llama/llama-3.3-70b-instruct,qwen/qwen3-next-80b-a3b-instruct
   ```
   Без суффикса `:free` — модели стоят порядка $0.05–$0.20 за миллион
   токенов, для типичной нагрузки сервиса это копейки в месяц.
